...
overwrite_all() {
    ...
    # Perform fresh installation (includes global command installation)
...
# Install global agent-os command
install_global_command() {
    local bin_dir="/usr/local/bin"
    local command_path="$bin_dir/agent-os"

    print_status "Installing global agent-os command..."

    # Check if we need sudo
    if [[ ! -w "$bin_dir" ]]; then
        echo ""
        echo -e "${YELLOW}Installing global command requires sudo access${NC}"

        # Create the command script content
        local script_content='#!/bin/bash
# Agent-OS Global Command
# Auto-generated by Agent OS base installation
...
case "$1" in
    -i)
        bash "$SCRIPTS_DIR/project-install.sh"
        ;;
    -u)
        bash "$SCRIPTS_DIR/project-update.sh"
        ;;
    *)
        echo "Use: agent-os [-i | -u]"
        echo ""
        echo "Options:"
        echo "  -i    Install Agent OS in current project directory"
        echo "  -u    Update Agent OS in current project directory"
        ;;
esac
'

        # Try to install with sudo
        if echo "$script_content" | sudo tee "$command_path" > /dev/null 2>&1; then
            sudo chmod +x "$command_path" 2>/dev/null

            if [[ -x "$command_path" ]]; then
                echo "✓ Global agent-os command installed at $command_path"
                echo ""
                print_success "You can now use 'agent-os' from anywhere!"
                return 0
            fi
        fi

        # If sudo installation failed
        print_warning "Could not install global command (sudo required or failed)"
        echo ""
        echo -e "${YELLOW}To install manually, run:${NC}"
        echo -e "  sudo bash -c 'cat > $command_path << \"EOF\"
#!/bin/bash
AGENT_OS_DIR=\"\$HOME/agent-os\"
SCRIPTS_DIR=\"\$AGENT_OS_DIR/scripts\"

case \"\$1\" in
    -i) bash \"\$SCRIPTS_DIR/project-install.sh\" ;;
    -u) bash \"\$SCRIPTS_DIR/project-update.sh\" ;;
    *) echo \"Use: agent-os [-i | -u]\" ;;
esac
EOF
'
  sudo chmod +x $command_path"
        ...
        # If we have write access, install directly
...
perform_fresh_installation() {
    ...
    echo -e "   ${YELLOW}agent-os -i${NC}"
    echo -e "   ${YELLOW}(or ~/agent-os/scripts/project-install.sh)${NC}"
    echo ""
